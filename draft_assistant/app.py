...
st.set_page_config(page_title="FF Draft Assistant — Dynamic Strategy, VBD & Pivots", layout="wide")

# --------------------------
# Env/config knobs
# --------------------------
DEFAULT_CSV_PATH = os.environ.get("FFDA_CSV_PATH", "")
QB_ROSTER_CAP = int(os.environ.get("FFDA_QB_CAP", "2"))  # cap QB suggestions to keep max 2 on roster
INCLUDE_K_DEF_EARLY = bool(int(os.environ.get("FFDA_INCLUDE_K_DEF_EARLY", "0")))
ADP_GUARD_TOL = int(os.environ.get("FFDA_ADP_GUARD_TOL", "6"))  # ADP guard tolerance (strong default)
...
def live_tab(csv_df, weights, league_id, username, seat_override, poll_secs, auto_live, include_k_def_anytime):
    ...
    # Dynamic strategy selection (every pick)
    ...
    # Enforce QB cap, ensure K/DEF when needed
...
# --------------------------
# MOCK TAB (keep board visible here)
# --------------------------
...
# --------------------------
# BOARD TAB (standalone board & filters)
# --------------------------
